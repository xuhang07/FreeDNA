...
def main():
    ...
    noise_variance_list = np.mean(np.load('./Laion_variance_list.npy'),axis=0)
    # noise_variance_list = None
    set_seed(args.seed)
    if args.version == "v1":
        V1InferenceLoop(args).run(noise_variance_list)
    else:
        supported_tasks = {
            "sr": BSRInferenceLoop,
            "dn": BIDInferenceLoop,
        ...
        supported_tasks[args.task](args).run(noise_variance_list)
        print("done!")
