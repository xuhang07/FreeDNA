...
class InferenceLoop:
    ...
    def after_load_lq(self, lq: np.ndarray) -> np.ndarray:
        ...
        # #         )
        # h, w, _ = lq.shape
        # # img_lq = cv2.filter2D(lq, -1, kernel)
        # # downsample
        # img_lq = lq
        # scale = 4
        # img_lq = cv2.resize(img_lq, (int(w // scale), int(h // scale)), interpolation=cv2.INTER_LINEAR)
        # # noise
        # img_lq = random_add_gaussian_noise(img_lq, [5, 5])
        # # jpeg compression
        # img_lq = random_add_jpg_compression(img_lq, [50, 50])
    ...
    def run(self,noise_variance_list: List[float]) -> None:
        ...
        for lq in loader():
            ...
            sample = sample[0]
            # print(np.mean(np.var(sample,axis=0)))
            self.save(sample)
