...
class Pipeline:
    ...
    def run_stage2(
        ...
        cfg_scale: float,
        better_start: float,
        noise_variance_list: List[float]
    ) -> torch.Tensor:
        ### preprocess
        # clean = clean.repeat(5,1,1,1)
        ...
        if better_start:
            ...
            x_T = torch.randn((bs, 4, h // 8, w // 8), dtype=torch.float32, device=self.device)*1.0
        ### run sampler
        ...
        z = sampler.sample(
            ...
            progress_leave=True, cond_fn=self.cond_fn, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride, noise_variance_list=noise_variance_list
        )
        if not tiled:
    ...
    def run(
        ...
        sample= self.run_stage2(
            ...
            pos_prompt, neg_prompt, cfg_scale, better_start, noise_variance_list
        )
