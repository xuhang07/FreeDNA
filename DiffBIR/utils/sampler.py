...
class SpacedSampler(nn.Module):
    ...
    def predict_noise(
        ...
        if uncond is None or cfg_scale == 1.:
            ...
            model_output = cfg_scale * model_cond + (1 - cfg_scale) * model_uncond
        return model_output
    ...
    def p_sample(
        ...
        delta_N_1 = np.sqrt((eps**2).mean().cpu().numpy()/noise_variance).item()
        scale = 1
        if delta_N_0 is not None:
            scale = delta_N_1-delta_N_0+1-scale_sum
        else:
            scale = 1
        pred_x0 = self._predict_xstart_from_eps(x, index, eps*1.02)
        if cond_fn:
    ...
    def sample(
        ...
        noise_variance_list: List[float]=None,
    ) -> torch.Tensor:
        ...
        for i, step in enumerate(iterator):
            ...
            if noise_variance_list is None:
                delta_N_0 = None
                noise_variance = 1
            else:
                noise_variance = noise_variance_list[i]
            img,scale,delta_N_1 = self.p_sample(
                model, img, ts, index, cond, uncond, cfg_scale, cond_fn,
                tiled, tile_size, tile_stride, noise_variance,delta_N_0,scale_sum
            )
            delta_N_0 = delta_N_1
            scale_sum += scale-1
